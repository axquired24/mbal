    ...
    <div class="row justify-content-center mb-4">
        <div class="input-group col-md-12 justify-content-center">
            <form class="form-inline my-2 my-lg-0" type="get" action="{{ url('/search') }}">
                <input class="form-control mr-sm-2 {{ $errors->has('query') ? 'is-invalid' : ''}}" style="width:350px;"  name="query"   type="search" placeholder="Search" aria-label="Search">
                @if ($errors->has('query'))
    ...
    <div class="accordion" id="accordionExample">
        ...
        <div class="card">
            <div class="card-header" id="headingOne">
                <h2 class="mb-0">
                    <button class="btn btn-link" type="button" data-toggle="collapse" data-target="#collapse{{$tahun->tahun}}" aria-expanded="true" aria-controls="collapseOne">
                        {{$tahun->tahun}}
                    </button>
                </h2>
            </div>

            <div id="collapse{{$tahun->tahun}}" class="collapse show" aria-labelledby="headingOne" data-parent="#accordionExample">
                            ...
                            <div class="ml-auto">    
                                <div class="btn-group" role="group" aria-label="Basic example">
                                    <form class="form-delete mr-2" action="{{ route('destroy', $document->id) }}" method="post">
                                        @method('DELETE')
                                        @csrf
                                        <button type="submit" class="btn btn-outline-danger btn-sm" onclick="return confirm('Apakah yakin akan menghapus dokumen {{$document->title}}?')">
